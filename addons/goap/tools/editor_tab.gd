...
const ATOM_ICONS = [
	preload("res://addons/goap/icons/atom_false.png"),
	preload("res://addons/goap/icons/atom_true.png"),
	preload("res://addons/goap/icons/atom_dontcare.png")
]
...
func edit(ap: GOAPActionPlanner):
	clear()
	$VBoxContainer/HBoxContainer/NewAction.disabled = false
...
func clear():
	$VBoxContainer/HBoxContainer/NewAction.disabled = true
...
func on_action_selected():
	...
	if action != null:
		preconditions_list.update_list(
			action_planner.state_atoms, action_planner.parse_state(action.preconditions)
		)
		effect_list.update_list(
			action_planner.state_atoms, action_planner.parse_state(action.effect)
		)
